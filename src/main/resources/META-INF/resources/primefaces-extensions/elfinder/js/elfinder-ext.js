PrimeFacesExt.widget.ElFinder = PrimeFaces.widget.BaseWidget.extend({
	...
	init : function(cfg) {
		...
		var options = {
			...
			lang : 'ru',
			handlers : {
				//elfinderInstance.options.commands
				init : function(event, elfinderInstance) {
					elfinderInstance.options.prime.fireEvent(event.type);
				},
				...
				dblclick : function(event, elfinderInstance) {
					elfinderInstance.options.prime.fireEvent('dblclick');
				},
				...
				changeclipboard : function(event, elfinderInstance) {
					elfinderInstance.options.prime.fireEvent('changeclipboard');
				},
				...
				duplicate : function(event, elfinderInstance) {
					elfinderInstance.options.prime.fireEvent('duplicate');
				},
				download : function(event, elfinderInstance) {
					elfinderInstance.options.prime.fireEvent('download');
				},
				...
				contextmenu : function(event, elfinderInstance) {
					elfinderInstance.options.prime.fireEvent('contextmenu');
				},
				...
				viewchange : function(event, elfinderInstance) {
					elfinderInstance.options.prime.fireEvent('viewchange');
				},
				sortchange : function(event, elfinderInstance) {
					elfinderInstance.options.prime.fireEvent('sortchange');
				},
				searchstart : function(event, elfinderInstance) {
					elfinderInstance.options.prime.fireEvent('searchstart');
				},
				...
				searchend : function(event, elfinderInstance) {
					elfinderInstance.options.prime.fireEvent('searchend');
				},
		...
		var elfinder = $(PrimeFaces.escapeClientId(this.id)).elfinder(options).elfinder('instance');
		elfinder['cfg'] = cfg;
	...
	fireEvent : function(eventName, cmd) {
		if (this.cfg.behaviors) {
			var callback = this.cfg.behaviors[eventName];
			if (callback) {
				var ext = {
